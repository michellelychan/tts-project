...
import mlflow  # Changed from wandb to mlflow
import os
import torch
...
mlflow.set_experiment("speecht5-tts-finetuning")  # Set experiment name

# Start an MLflow run
run = mlflow.start_run(run_name="speecht5_tts_finetuning")
...
# Log unsupported characters as text
...
def generate_audio_samples(model, processor, dataset, vocoder, num_samples=3):
    ...
    for i, example in enumerate(examples):
        ...
        # Also save the caption
        caption_path = f"../outputs/audio/sample_{i+1}_{timestamp}_caption.txt"
        with open(caption_path, "w") as f:
            f.write(caption)
        mlflow.log_artifact(caption_path, artifact_path="audio_captions")
...
# Update the active MLflow run with more metadata
...
training_args = Seq2SeqTrainingArguments(
    ...
    eval_steps=1000,
    logging_steps=25,
    report_to=["tensorboard", "mlflow"],  # Changed from wandb to mlflow
...
# Create a custom MLflow callback to register the model and generate audio samples
class ModelRegistryAndAudioCallback(MLflowCallback):
    ...
    def on_train_end(self, args, state, control, **kwargs):
        ...
        print(f"✅ Model registered in MLflow as: speecht5_finetuned_nl_{timestamp}")
...
# Log the processor files
mlflow.log_artifacts(processor_output_path, artifact_path="processor")
...
print(f"Model registered in MLflow as: speecht5_finetuned_nl_{timestamp}")
print(f"To use this model, load it from MLflow or from the local directory: {output_dir}")
